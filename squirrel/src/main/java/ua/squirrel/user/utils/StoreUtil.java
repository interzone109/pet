			...
			ingridientNode.forEach(node->{
				if(consignmentData.containsKey(node.getProduct().getId())){
					...
					if (sing.equals("+")) {
						...
						System.err.println(node.getLeftOvers() +"+"+ Integer.parseInt(data[1])+"="+leftover);
			...
			ingridientNode.forEach(node->{
				if(consignmentData.containsKey(node.getProduct())){ 
					if (sing.equals("+")) {
						...
						int unitePrice =   (node.getSumm() == 0 || node.getLeftOvers()==0 )?0 : node.getSumm() / node.getLeftOvers();
