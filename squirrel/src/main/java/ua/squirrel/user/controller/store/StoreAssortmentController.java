...
public class StoreAssortmentController {
	...
	/**
	 * Метод удаляет узел магазин-продукт
	 */
	@DeleteMapping("{product_id}")
	public ResponseEntity<String> delete(@PathVariable("store_id")Long storeId,@PathVariable("product_id")Long productId,
			Authentication authentication) throws NotFoundException {
		...
		if(storeNode.isPresent()) {
			...
			storeCompositeProductServiceImpl.deleteById(storeNode.get().getId());
	...
	*/
	...
	public List<CompositeProductModel> addToStoreProduct(@PathVariable("store_id") Long storeId ,
		...
		compositeProductServiceImpl.findAllByUserAndIdIn(user, newProductPrice.keySet()).forEach(compProd->{
			...
			System.err.println("price =  "+newProductPrice.get(compProd.getId()));
