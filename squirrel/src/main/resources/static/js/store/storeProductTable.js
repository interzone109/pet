...
// метод создает строку продукт - цена
 function createNewStoreProductRow(product){
	 ...
	 productRow.innerHTML = "<td id=\"product_group_id_"+product.id+"\">"+product.group+"</td>"
			 ...
			 + "<td id=\"product_price_id_"+product.id+"\">"+displayProductPrice( product.totalSumm)+"</td>"
			 ...
			 +"<span class=\"badge badge-info\"><i class=\"fas fa-list-alt\" title=\"вернуться к магазинам\" onclick=\"showStoreTable()\"  ></i></span>"
			 +" <span class=\"badge badge-danger\"><i class=\"fas fa-trash-alt\" title=\"удалить\"  onclick=\"removeProduct("+product.id+")\"></i></span></td>";
 ...
 //удаляем продукт с магазина
 function removeProduct(productId){
	 console.log("del");
	var storeId =  $('#compositeStoreId').text();
	$('#product_group_id_'+productId).text("удалено");
	
	 request("DELETE", connectUrl+"/user/stores/assortment/"+storeId+"/"+productId);
 }
