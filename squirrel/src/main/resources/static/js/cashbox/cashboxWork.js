...
function createNewStoreProductElem(product){
	 ...
	 productElem.innerHTML =  "<div class=\"card border-info mb-3 text-center\" style=\"max-width: 12rem; min-width: 12rem;\" >"
		 	...
		 	+"<div class=\" text-primary\" id=\"prod_measur_"+product.id +"\">"+displayProductMeasure(product.measureProduct,1)+"</div>"
		 //+"<div class=\" text\">"+product.name+"</div>"
		 		...
		 		+"<button class=\"btn btn-success col\"title=\""+product.totalSumm+"\" id=\"sell_button_id_"+product.id +"\" onclick=\"addProductToBusket("+product.id+")\">"
		 		+displayProductPrice(product.totalSumm) +"</button>"
		 	+"</div>"
		 +"</div>";
	 return productElem;
}
