...
public class StorageUtils {
	 ...
	 * 
	 * @param List<CompositeProduct> compositeProductList список сущностей 
	 * у которых берутся данные для модели
	 * @param  Map<Long, Integer> idsPrice мапа используеться для пpисвоения цены по id
	 * */
	public Map<CompositeProductModel, Integer> getCompositeProductModel
	( List<CompositeProduct> compositeProductList, Store store) {
		...
		compositeProductList.stream().forEach(prod -> {
			productPriceMap.put(
					...
					.name(prod.getName())
					.group(prod.getGroup())
					.propertiesProduct(prod.getPropertiesProduct().toString())
					.build(),
	 ...
	 * @param Map<Long, Integer> updateIdsPrice   мапа с новыми ценами: id-новая цена
	...
	public Store updateIdsPrice(Store store , Map<Long, Integer> updateIdsPrice ,
		...
		//проверяем не пустая ли строка с изменениями в цене и записываем в нее новые данные об изменениях
	 ...
	 * @param List<CompositeProduct> compositeProductList список совпадений 
	 * */
	public Store deleteIdsPrice(Store store , List<Long> delete ,
			List<CompositeProduct> compositeProductList) {
		...
		store.getStorage().setProductPrice(newPrice.toString());
		//проверяем на наличие ранее удаленных прдуктов и добавляем к ним новые удаления
